...
public:
    vector<vector<int>> getSkyline(vector<vector<int>> &buildings)
    {
        sort(buildings.begin(), buildings.end(), [](const vector<int> &a, const vector<int> &b)
             { return a[2] > b[2]; }); // sort buildings by highest height
        vector<vector<int>> ans;
        map<int, int> points;
        for (auto i : buildings)
        { // set up points of interest, at most 20000
            points[i[0]] = 0;
            points[i[1]] = 0;
        }
        ...
        for (auto i : buildings)
        {                                           // find all points passing through POIS
            ...
            while (it != unsearched_points.end() && it->first < i[1])
            ...
            { // delete searched points because they already have their max value
        ...
        for (auto i : points)
        {
            ...
            if (height != last_height)
                ans.push_back({id, height}); // look for change in height
            last_height = height;
        }
        return ans;
    }
